 ...
 * @param context
 */
export const sysmsgParser: MessageParser = async (engineMessage: MessagePayload, ret: PUPPET.payloads.Message, context: MessageParserContext) => {
  const sysmsgPayload = await parseSysmsgMessagePayload(engineMessage)
  if (!sysmsgPayload) {
    return ret
  }
