...
exports.WechatMessageType = exports.MsgSource = void 0;
...
class Client extends events_1.EventEmitter {
    ...
    initServer() {
        if (!this.app && !this.server) {
            ...
            this.app.use(body_parser_1.default.json({ limit: '200mb' }));
            this.app.use(body_parser_1.default.urlencoded({ extended: true }));
            this.server = http_1.default.createServer(this.app);
            const _port = this.options.port;
            this.server.listen(_port, () => {
                wechaty_puppet_1.log.info(PRE, `Server is running on ${_port}`);
            });
            this.eventListen();
        }
    }
    eventListen() {
        this.app.post('/wechat/', async (req, res) => {
            ...
            wechaty_puppet_1.log.info(PRE, `on event:${JSON.stringify(data)}`);
            switch (type) {
                case 'D0001':
                    this.emit('hook', { errorCode: 0, errorMessage: 'success' });
                    break;
                case 'D0002':
                    wechaty_puppet_1.log.info(PRE, 'login event');
                    this.emit('login', { errorCode: 0, errorMessage: 'success', name: data.nick, wxid });
                    break;
                case 'D0003': {
                    wechaty_puppet_1.log.info(PRE, 'recive message');
                    const msg = {
                        ...data,
                        text: data.msg,
                ...
                case 'D0004': {
                    wechaty_puppet_1.log.info(PRE, 'transfer message');
                    const transferMsg = {
                        ...data,
                        id: (0, cuid_1.default)(),
                        listenerId: wxid,
                ...
                case 'D0005':
                    wechaty_puppet_1.log.info(PRE, 'recall message');
                    break;
                case 'D0006': {
                    wechaty_puppet_1.log.info(PRE, 'friend request');
                    const friendShip = {
                        ...data,
                        contactId: data.wxid,
                        hello: data.content,
    ...
    async postData(data) {
        try {
            ...
            if (parseInt(result.code) === 200) {
                ...
                wechaty_puppet_1.log.info(PRE, `Error: onPost${result}`);
                return { code: Number(result.code), errorMsg: result.msg, result: result.result };
            }
        }
        catch (e) {
            wechaty_puppet_1.log.error('post error', e);
        }
    }
    /**
     * 获取微信运行状态
    ...
    async getStats() {
        ...
        if (res) {
            ...
            wechaty_puppet_1.log.info(PRE, 'login fail');
     ...
     */
    async getImage(img) {
        return file_box_1.FileBox.fromUrl(`${this.options.httpServer}/DaenWxHook/client/view/?name=${img}`);
    }
    /**
     * 获取图片数据
     */
    async getFile(name) {
        ...
        const res = await axios_1.default.get(this.options.httpServer + '/DaenWxHook/client/down/', {
            params: {
                name,
                date: `${year}-${month}`,
            },
